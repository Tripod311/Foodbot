...
def add_nutrient(alias, nutrient_name, daily_amount, products):
    cur = conn.execute(f"""CREATE TABLE IF NOT EXISTS {alias} (
product_id INTEGER,
amount REAL);""")
    cur.close()
    cur = conn.execute("INSERT INTO nutrients (name, alias, daily) VALUES (?, ?, ?)", [nutrient_name, alias, daily_amount])
    cur.close()
...
for nutrient in data:
    ...
    add_nutrient(alias, name, daily, products)
#
conn.commit()
